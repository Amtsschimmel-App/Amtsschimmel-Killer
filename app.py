...
def get_ai_analysis(text):
    try:
        ...
        sys_msg = (
            ...
            "Das Antwortschreiben und der Widerspruch müssen detailliert sein und folgende Platzhalter am Ende enthalten: "
...
def create_word_complete(ana, ant, wid):
    ...
    for t, c in [("Analyse", ana), ("Antwort", ant), ("Widerspruch", wid)]:
        doc.add_heading(t, level=1); doc.add_paragraph(str(c))
    target = BytesIO()
    doc.save(target)
...
with col_right:
    ...
    if 'res' in st.session_state:
        ...
        erhalt = st.date_input("Erhalt-Datum wählen", datetime.now())
        ...
        with t2: st.text_area("Antwort:", r['antwort'], height=350)
        with t3: st.text_area("Widerspruch:", r['widerspruch'], height=350)
        
        st.divider()
        st.download_button("📥 Excel Download", create_excel_pro(r['analyse'], r['antwort'], r['widerspruch']), "Analyse.xlsx")
        st.download_button("📥 Word Download", create_word_complete(r['analyse'], r['antwort'], r['widerspruch']), "Analyse.docx")
    else:
