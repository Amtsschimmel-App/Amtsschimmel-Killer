...
# --- 4. TOP-BAR: RECHTLICHES (MIT GROSSEN ABSTÄNDEN) ---
...
with col_mid:
    ...
    uploaded_file = st.file_uploader("Brief hochladen", type=['pdf', 'png', 'jpg', 'jpeg'], label_visibility="collapsed")
...
with col_right:
    ...
    if 'analysis_results' in st.session_state:
        ...
        with t2: st.text_area("Entwurf:", res['antwort'], height=300)
        with t3: st.text_area("Entwurf:", res['widerspruch'], height=300)
        st.divider()
        st.download_button("📥 PDF Download", create_pdf_adobe_ready(res['analyse'], res['antwort'], res['widerspruch']), "Analyse.pdf", use_container_width=True)
    else:
