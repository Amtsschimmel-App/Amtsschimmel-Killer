...
from docx import Document # Wichtig: python-docx muss in requirements.txt stehen
import io
import os
import pandas as pd
import re
...
Ihre Dokumente werden verschlüsselt an die KI übertragen, dort nur kurzzeitig verarbeitet und niemals dauerhaft gespeichert. Nach der Analyse werden die Daten gelöscht.
...
Sehr geehrte Damen und Herren, gegen Ihren Bescheid vom [Datum], erhalten am [Datum], lege ich hiermit Widerspruch ein. Eine detaillierte Begründung folgt. Mit freundlichen Grüßen, [Name]
**Akteneinsicht:**  
...
with col_left:
    ...
    u_file = st.file_uploader("Datei hochladen", type=['png', 'jpg', 'pdf'])
    if u_file:
...
with col_right:
    ...
    if st.session_state.full_res:
        ...
        with ex4: st.download_button("📅 Kalender", create_ics_final(st.session_state.full_res), "Termine.ics")
    else:
        st.info("Ergebnis folgt nach Scan.")
