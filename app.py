...
def create_word_complete(ana, ant, wid):
    ...
    for t, c in [("1. Analyse", ana), ("2. Antwort", ant), ("3. Widerspruch", wid)]:
...
# --- 4. TOP-BAR: RECHTLICHES (GROSSE ABSTÄNDE & KOMPLETTER TEXT) ---
...
with col_right:
    ...
    if 'res' in st.session_state:
        ...
        st.info(f"Ihre Frist endet voraussichtlich am: **{(erhalt + timedelta(days=30)).strftime('%d.%m.%Y')}**")
        
        st.write("---")
        t_ana, t_ant, t_wid = st.tabs(["Analyse", "Antwortbrief", "Widerspruch"])
        with t_ana: st.write(r['analyse'])
        with t_ant: st.text_area("Entwurf Antwort:", r['antwort'], height=350)
        with t_wid: st.text_area("Entwurf Widerspruch:", r['widerspruch'], height=350)
