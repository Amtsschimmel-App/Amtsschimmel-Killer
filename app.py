...
def get_ai_analysis(text):
    """Echte KI-Analyse über OpenAI."""
    try:
        client = openai.OpenAI(api_key=st.secrets["OPENAI_API_KEY"])
        response = client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "Du bist ein Experte für deutsches Verwaltungsrecht. Analysiere den Brief, erkenne Fristen und erstelle ein ausführliches Antwortschreiben sowie einen detaillierten Widerspruch. Antworte nur in JSON: {'analyse': '...', 'antwort': '...', 'widerspruch': '...', 'frist': 'DD.MM.YYYY'}"},
...
def create_pdf_adobe_ready(analyse, antwort, widerspruch):
    ...
    for title, content in sections:
        ...
        safe_text = str(content).replace('•', '-').replace('–', '-').replace('„', '"').replace('“', '"').replace('”', '"').replace('’', "'")
...
def create_excel_pro(ana, ant, wid):
    output = BytesIO()
    data = [{"Kategorie": "1. Analyse", "Inhalt": ana}, {"Kategorie": "2. Antwort", "Inhalt": ant}, {"Kategorie": "3. Widerspruch", "Inhalt": wid}]
...
with col_right:
    ...
    if uploaded_file:
        ...
        st.error(f"📅 FRIST ERKANNT: {res.get('frist', 'Keine Frist gefunden')}")
        st.info(res.get('analyse'))
        
        tab1, tab2, tab3 = st.tabs(["✍️ Antwort", "⚖️ Widerspruch", "📥 Downloads"])
        with tab1: st.text_area("Entwurf Antwort:", res.get('antwort'), height=280)
        with tab2: st.text_area("Entwurf Widerspruch:", res.get('widerspruch'), height=280)
        with tabs:
            st.download_button("📊 Excel-Bericht", create_excel_pro(res['analyse'], res['antwort'], res['widerspruch']), "Analyse.xlsx")
            st.download_button("📝 Word-Dokument (Komplett)", create_word_complete(res['analyse'], res['antwort'], res['widerspruch']), "Bericht.docx")
